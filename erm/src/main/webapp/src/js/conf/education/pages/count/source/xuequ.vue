      ...
      <Card :padding="10">
        <div class="xuequ">
          <div class="chart" v-if="listData.length">
            <Row>
              <Col span="24">
                <div id="chart-con" style="height: 400px;"></div>
              </Col>
            </Row>
          </div>
...
import $ from 'jquery'
import { bar } from 'js/lib/echarts/index'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      io.get(api.report.school, searchData, (res) => {
        let data = res.data
        ...
        this.$nextTick(() => {
          bar('chart-con', {
            xAxis: [{
              data: data.echarts.xData
            }],
            series: [
              {
                name: '资助金额',
                type: 'bar',
                data: data.echarts.yData
              }
            ]
          })
        })
    ...
    // 切换
    goWhere (o) {
      this.$router.push({ name: 'count/renyuan', params: { year: o.year }, query: { schoolType: o.school_type_id } })
    }
  },
  mounted () {
    this.getList()
