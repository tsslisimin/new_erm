      ...
      <Card :padding="10">
        ...
        <v-table v-if="listData.length" :nowrap="true" :items="listData" :header="[]">
          <template slot="t-header">
            ...
            <th class="center"><p sort-key="funded_name"><span>项目名称</span><span class="sort"><i class="iconfont up">&#xe6f4;</i><i class="iconfont down">&#xe608;</i></span></p></th>
          </template>
          <template slot="t-body" slot-scope="props">
            <tr>
              <td class="left">{{props.idx + 1 + 10 * (page.page - 1)}}</td>
              <td class="center">
                <router-link v-if="props.data.funded_id" :to="{ name: 'detail', params: { stuId: props.data.id, fundId: props.data.funded_id } }">{{props.data.name}}</router-link>
                <router-link v-if="!props.data.funded_id" :to="{ name: 'studentDetail', params: { stuId: props.data.id } }">{{props.data.name}}</router-link>
              </td>
