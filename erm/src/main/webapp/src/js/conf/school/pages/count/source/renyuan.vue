            ...
            <Row :gutter="10">
              ...
              <Col span="6">
                <Select v-model="formList.fund" filterable placeholder="请选择">
                  <Option v-for="item in fundList" :value="item.id" :key="item.id">{{item.fundedName}}</Option>
                </Select>
              </Col>
  ...
  methods: {
    ...
    getList () {
      let searchData = _.merge({}, this.page, {
        ...
        schFundedId: this.formList.fund
      })
    ...
    // 获取项目列表
    getProjectList () {
      io.get(api.schoolFunded.listSimple, (res) => {
        let data = res.data
        this.fundList = data.page.list
      })
    },
    // 获取年份列表
    getYearList () {
